...
def sft_training_set():
    aspects = ["观察：", "感受：", "需求：", "请求："]

    with open(f'{ROOT_DIR}/datasets/EmpatheticLLMs/PsyDTCorpus_cot/response_cot.jsonl', 'r') as f:
        dataset = [json.loads(line) for line in f.readlines()]
    nvc_path = f'{ROOT_DIR}/datasets/EmpatheticLLMs/PsyDTCorpus_train/contrastive_train.json'
    if not os.path.exists(nvc_path):
        formate_nvc()
    nvc_component = json.load(open(nvc_path, 'r'))

    json_dataset = []
    for i, line in tqdm(enumerate(dataset), total=len(dataset), desc='Processing vanilla cot response data'):
        user_state = nvc_component.pop(0).get('user_state', '')
        if line['label'] == 'dataset':
            ...
            if user_state:
                ...
                cot = strategy
            conversation = '\n\t'.join([f'{ROLE_MAP[msg["role"]]}: {msg["content"]}' for msg in line['conversation']])
...
def main(function: str):
    if function == 'train_simulator':
        simulator_training_set()
    elif function == 'train_sft':
        sft_training_set()
