...
class BatchedSimulatedConv:
    def __init__(
        ...
        with Pool(processes=self.bsize) as pool:
            results = pool.map(self.step_wrapper, zip(conv_list, user_descs, replies, repeat("simulator")))
        self.conv_list = [[conv] for done, conv in results]
    ...
    def step_wrapper(self, args):
        conv, state, response, role = args
        done = conv.step(state, response, role)
        return done, conv
    ...
    def step(self, step_num: int):
        ...
        with Pool(processes=None) as pool:
            results = pool.map(self.step_wrapper, zip(self.conv_list, user_states, replies, repeat(role)))

        progress, self.conv_list = zip(*results)
        # with concurrent.futures.ThreadPoolExecutor() as executor:
