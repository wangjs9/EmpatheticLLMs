...
def create_simulator_model(
        model: "AutoModelForCausalLMWithValueHead", model_args: "ModelArguments", finetuning_args: "FinetuningArguments"
) -> Optional["AutoModelForCausalLMWithValueHead"]:
    r"""
    Creates simulator model for PPO training.
    """
    if finetuning_args.simulator_model_type == "api":
        assert finetuning_args.simulator_model.startswith("http"), "Please provide full url."
        logger.info_rank0(f"Use simulator server {finetuning_args.simulator_model}")
        return finetuning_args.simulator_model
    elif finetuning_args.simulator_model_type == "lora":
        model.pretrained_model.load_adapter(finetuning_args.simulator_model, "simulator")
        for name, param in model.named_parameters():
            if "default" in name:
                param.data = param.data.to(torch.float32)
        vhead_params = load_valuehead_params(finetuning_args.simulator_model, model_args)
        assert vhead_params is not None, "Simulator model is not correctly loaded."
        model.register_buffer("simulator_head_weight", vhead_params["v_head.summary.weight"], persistent=False)
        model.register_buffer("simulator_head_bias", vhead_params["v_head.summary.bias"], persistent=False)
        ...
        logger.info_rank0(f"Loaded adapter weights of simulator model from {finetuning_args.simulator_model}")
        return None
    else:
        simulator_model_args = ModelArguments.copyfrom(
            model_args,
            model_name_or_path=finetuning_args.simulator_model,
            adapter_name_or_path=finetuning_args.simulator_model_adapters,
            quantization_bit=finetuning_args.simulator_model_quantization_bit,
        )
        tokenizer = load_tokenizer(simulator_model_args)["tokenizer"]
        simulator_model = load_model(
            tokenizer, simulator_model_args, FinetuningArguments(), is_trainable=False, add_valuehead=True
        )
        logger.info_rank0(f"Loaded full weights of simulator model from {finetuning_args.simulator_model}")
        logger.warning_rank0("Please ensure the ppo model and simulator model share SAME tokenizer and vocabulary.")
