...
from typing import Dict, List, Sequence, Tuple, Any, Union
...
class ScoreTemplate:
    ...
    def _parse_example(self, example: Dict[str, Union[List[dict], str]]) -> Tuple[str, str]:
        r"""
        input: a dict with keys {"context", "condition", "response"}
        output: a tuple of (prompt, response)
        """
        conversation = self.context.format(
            conversation="\n".join(
                [f"{ROLE_MAP[turn['role']]}: {turn['content']}" for turn in example["conversation"]]))
        if self.condition == None or example['user_state'] == None:
            return conversation + self.response, example['response']
        else:
            user_state = self.condition.format(user_state=example['user_state'])
            return conversation + user_state + self.response, example['response']
    
    def format_example(
            self, target_data: Dict[str, str], support_set: Sequence[Dict[str, str]] = None
    ) -> List[Dict[str, str]]:
        r"""
        Converts dataset examples to messages.
        """
...
templates: Dict[str, Union["ScoreTemplate", "COTTemplate"]] = {}
...
def _register_template(name: str, system: str, context: str, condition: str = None, response: str = None) -> None:
    if '_cot' in name:
        ...
        templates[name] = ScoreTemplate(system=system, context=context, condition=condition, response=response)
    elif '_infer' in name:
...
def get_template(name: str) -> Union["ScoreTemplate", "COTTemplate", "InferTemplate"]:
...
_register_template(
    ...
    system='任务：\n扮演一位与来访者对话的倾听者，描述倾听者在回复的思考过程，最终补充完整的思维链（……部分）。\n\n思维链内容：\n1. 倾听者根据历史对话认为应该关注的访者状态（观察、情绪、需求或者请求），这个关注点直接影响倾听者的后续回复；\n2. 倾听者计划的回复策略（例如：建议、教育、安慰、回忆、否定、同情、询问等）和意图。\n\n要求：\n1. 视角：以倾听者计划回复的视角与口吻展开分析；\n2. 描述：详细说明倾听者回复背后的思维链；\n3. 思维过程：\n - 基于与来访者的对话历史作出推导；\n - 在推导过程中，倾听者不应预知后续回复的具体内容；\n - 通过思维链能够自然推导得出后续回复。\n\n',
    context='【历史对话】：\n{conversation}\n\n【倾听者思维链】：\n......\n\n【倾听者回复】：\n{response}\n\n【反例回复】：\n{contrast}\n\n',
    response='问题：【倾听者思维链】（......部分）是什么才能使得最终回复为【倾听者回复】{response}而不是【反例回复】{contrast}？\n\n答案：【倾听者思维链】为：\n'
)
